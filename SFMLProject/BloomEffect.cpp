...
void BloomEffect::apply(const sf::RenderTexture& input, sf::RenderTarget& output) {
	...
	// 2nd parameter is "bloom" factor; add the 1st and the bloom-> result stores in 3rd
	add(mFirstPassTextures[0], mSecondPassTextures[0], mFirstPassTextures[1]);
	mFirstPassTextures[1].display();
	// this is used as the new bloom, add with input->result to output
...
void BloomEffect::prepareTextures(sf::Vector2u size) {
	if (mBrightnessTexture.getSize() != size) {
		...
		mFirstPassTextures[0].create(size.x / 2, size.y / 2); // mFirstPassTexture is after the first downsampling
...
void BloomEffect::filterBright(const sf::RenderTexture& input, sf::RenderTexture& output) {
	...
	// setParameter passes values from Cpp to Shader program
