...
namespace {
	// Sound coordinate system, point of view of a player is in front of the screen
	...
	mSounds()
{
	...
	// Z of audio is outside of screen - opposite of graphics' Z axis
...
void SoundPlayer::removeStoppedSounds() {
	mSounds.remove_if([](const sf::Sound& s) {
		return s.getStatus() == sf::Sound::Stopped; //after Sound has finished playing
		});
}

void SoundPlayer::setListenerPosition(sf::Vector2f position) {
	// graphics is X=Right Y=Down; audio is X=Right Y=Up Z=OutOfScreen
	sf::Listener::setPosition(position.x, -position.y, ListenerZ);
}

// Get Listener position in Graphics space (instead of Audio space)
